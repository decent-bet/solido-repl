...
repl.start({
    ...
    context: {
        ...
        connexql: solidoBootstrap_1.getConnexql(env),
    },
}).then(async (instance) => {
    instance.context.mpp = solidoBootstrap_1.getMpp(env);
    instance.context.contracts = await solidoBootstrap_1.setupSolido(env, mapping_config_1.default);
    instance.context.connexql = await solidoBootstrap_1.getConnexql(env);
